...
int main()
{ 
  bool es71 = false;  // set true per stampare i valori istantanei (esercizio7.1)
  Input(); //Inizialization
  int nconf = 1;

  for(int istep=1; istep <= 500; ++istep) Move(); //Lascio equilibrare il sistema per 500 step

  if(es71 == true){
     
     ofstream ist;
     ist.open("7.1.ist_values.dat", ios::app);
     for(int istep=1; istep <= 500000; ++istep)
     {
       Move();
       Measure();
       ist << setw(8) << istep << "\t" << "Energia potenziale = " << walker[iv]/(double)npart + vtail << "\t" << "Pressione = " << rho * temp + (walker[iw] + ptail * (double)npart) / vol << endl;
       cout << istep << endl;
     }
  ...
  for(int iblk=1; iblk <= nblk; ++iblk) //Simulation
  {
    ...
    Averages(iblk, nstep);   //Print results for current block 
  }
...
//update of the histogram of g(r) -> identifica qual è il bin corretto e lo incremento di 2, perché questo doppio ciclo cicla solo una volta sulle coppie. Metti una condizione di controllo che dr sia < L/2
     if(dr <= box/2.){	
	for(int n = 0; n < 100; ++n){
...
void Averages(int iblk, int nstep) //Print results for current block
{
    
   double r;
   ofstream Gofr, Gave, Epot, Pres, unc;
   const int wd=12;
	...
	Gofr << setw(wd) << iblk << setw(wd) << k << setw(20) << stima_g << setw(20) << glob_av[k]/(double)iblk << setw(20) << err_gdir << endl;
