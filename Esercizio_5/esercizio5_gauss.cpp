...
int main (int argc, char *argv[]){
	...
	double r_medio[N] = {0.};
	ofstream fout("gauss_punti_100", ios::app);

	// T GAUSSIANA
	// psi_100
	cout << "Valor medio di psi_100 con T gaussiana " << endl;
	...
	for(int j = 0; j < 200; ++j){
		...
		double r = rnd.Gauss(0., 1.8*a_bohr);
	...
	for(int i = 0; i < N; ++i){
		...
		cout << sum_A/(double(L)) << endl;
	}

	fout.close();
	MediaBlocchi(N, r_medio, "gauss_r100", "gauss_r100_err");

	// psi_210
	cout << "Valor medio di psi_210 con T gaussiana " << endl;
	traj_iniziale[0] = 0;
	traj_iniziale[1] = 0;
	traj_iniziale[2] = a_bohr*5.;
	ofstream fout2("gauss_punti_210", ios::app);
	...
	for(int j = 0; j < 200; ++j){
		...
		double f = rnd.Rannyu();
		double theta = acos(1. - 2.*f);
		double r = rnd.Gauss(0., 4.6*a_bohr);
	...
	fout2.close();
	MediaBlocchi(N, r_medio, "gauss_r210", "gauss_r210_err");
