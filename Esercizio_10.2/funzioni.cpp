...
Population :: Population(int num_i, vector<vector<double> > cities, int rank){
...
vector<double> Population :: GetIndividual(int ind){

	return pop[ind];
}

void Population :: SetIndividual(vector<double> P, int ind){

	for(int i = 0; i < N; ++i) pop[ind][i] = P[i];		
}
...
void Population :: SortingPopulation(){
	...
	for(int n = 0; n < num; ++n){
		L = Fitness(pop[n]);
		pop[n][N] = L;	//aggiungo a ciascun individuo l'informazione della sua lunghezza
	}
...
void Population :: BestPath(int rank){

	ofstream path;
	path.open("bestpath_rank" + to_string(rank) + ".dat");
...
void Population :: PrintL(vector<double> P, int rank){

	ofstream L;
	L.open("bestL_rank" + to_string(rank) + ".dat",ios::app);
...
void Population :: RandomSearch(int rank){
...
void Population :: Iteration(int rank){
...
vector<double> Population :: Move(vector<double> G, double beta){

	vector<double> P(N);
	for(int i = 0; i < N; ++i) P[i] = G[i];	

	double L_old = Fitness(P);

	double r = rnd.Rannyu();
	if(r < 0.33) P = PairPermutation(P);
	if(0.33 < r && r < 0.66) P = Permutation(P);
	else P = Inversion(P);
	
	double L_new = Fitness(P);
	double acc = min(1., exp(-beta*(L_new - L_old)));	//accettanza del Metropolis
