...
public:
  // constructors
  Population(int num_i, vector<vector<double>> cities);
  // destructor
  ~Population();
  // methods
  ...
  void BestPath();						//stampa su file il percorso migliore
  void PrintL(vector<double> P);				//stampa su file la lunghezza di un percorso
  ...
  void RandomSearch();						//metodo per operare una iterazione di random search senza crossover
  void Iteration();						//metodo per operare una iterazione con crossover
};
